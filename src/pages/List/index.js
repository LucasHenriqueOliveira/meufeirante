...
export default function List() {
  ...
  const handleChangeProduto = (produto) => {
    setProducts(
      products.map((item) =>
        item.id === produto.id ? { ...item, checked: !produto.checked } : item
      )
    );
  };
  ...
  const handleChangeTipo = (tipo) => {
    setTypes(
      types.map((item) =>
        item.id === tipo.id ? { ...item, checked: !tipo.checked } : item
      )
    );
  };
  ...
  useEffect(() => {
    ...
    api.get("regioes/municipios/" + ufLowerCase).then((response) => {
      ...
      response.data.forEach((element) => {
        municipios.push({
          id: element._id,
          name: capitalize(element.nome_municipio),
        });
      });
  ...
  const handleMessage = (tel) => {
    window.open(
      "https://api.whatsapp.com/send?phone=55" +
        tel +
        "&text=Olá!%20Gostaria%20de%20informações%20dos%20produtos.&source=&data=",
      "_blank"
    );
  };

  const handleMessageGoogle = (link) => {
    window.open("https://www.google.com.br/maps/search/" + link, "_blank");
  };
              ...
              >
                ...
                <Card className={classes.card}>
                  <CardContent className={classes.cardContent}>
                    ...
                    <Typography>{feirante.email}</Typography>

                    <Typography className={classes.legend}>Telefone</Typography>
                    <div>
                    ...
                    <div>
                      <Typography className={classes.legend}>Tipos</Typography>
                  ...
                  <CardActions
                    className={(classes.cardActions, classes.botaoLoad)}
                  >
                    <Button
                      size="small"
                      onClick={() => handleMessage(feirante.telefonePrincipal)}
                      color="primary"
                    >
                      <WhatsAppIcon style={{ color: "#06D755" }}></WhatsAppIcon>
                      Envie uma mensagem
                    </Button>
                  </CardActions>
                </Card>
